...
//
// Construct with NewBreadthFirstLabeled.
//
...
type BreadthFirstLabeled struct {
	Graph  LabeledAdjacencyList
	Rand   *rand.Rand
	Result FromList
}

// NewBreadthFirstLabeled creates a BreadthFirstLabeled object.
//
// Argument g is the graph to be searched, as an adjacency list.
// Graphs may be directed or undirected.
//
// The graph g will not be modified by any BreadthFirstLabeled methods.
// NewBreadthFirstLabeled initializes the BreadthFirstLabeled object for the
// order (number of nodes) of g.  If you add nodes to your graph, abandon any
// previously created BreadthFirstLabeled object and call
// NewBreadthFirstLabeled again.
//
// Searches on a single BreadthFirstLabeled object can be run consecutively
// but not concurrently.  Searches can be run concurrently however, on
// BreadthFirstLabeled objects obtained with separate calls to
// NewBreadthFirstLabeled, even with the same graph argument to
// NewBreadthFirstLabeled.
func NewBreadthFirstLabeled(g LabeledAdjacencyList) *BreadthFirstLabeled {
	return &BreadthFirstLabeled{
		Graph:  g,
		Result: NewFromList(len(g)),
	}
}

// BreadthFirstLabeledPath finds a single path from start to end with a minimum
// number of nodes.
//
// Returned is the path as list of nodes.
// The result is nil if no path was found.
func (g LabeledAdjacencyList) BreadthFirstLabeledPath(start, end NI) []NI {
	b := NewBreadthFirstLabeled(g)
...
// from b.Result.
func (b *BreadthFirstLabeled) Path(start, end NI) bool {
	b.Traverse(start, func(n NI) bool { return n != end })
...
// d.Result.
func (b *BreadthFirstLabeled) AllPaths(start NI) int {
...
func (b *BreadthFirstLabeled) Traverse(start NI, v Visitor) int {
	b.Result.reset()
	rp := b.Result.Paths
